...
class PositionVector(object):
    ...
    def __init__(self,initial_x=0,initial_y=0,initial_z=0):
        # default config file
        self.posvec_home_location = LocationGlobalRelative(0, 0, 0)
        self.posvec_lon_scale = 1.0
        self.posvec_latlon_to_m = 111319.5
        self.x = initial_x
        self.y = initial_y
        self.z = initial_z
    ...
    def get_location(self):
        dlat = self.x / self.posvec_latlon_to_m
        dlon = self.y / (self.posvec_latlon_to_m * self.posvec_lon_scale)
        return LocationGlobalRelative(dlat, dlon, self.z)
    ...
    def set_from_location(self, location):
        ...
        self.x = (location.lat - self.posvec_home_location.lat) * self.posvec_latlon_to_m
        self.y = (location.lon - self.posvec_home_location.lon) * self.posvec_latlon_to_m * self.posvec_lon_scale
        self.z = location.alt

    # set_home_location - sets home location used by all position vector instances
    @classmethod
    def set_home_location(cls, home_location):
        self.posvec_home_location = home_location
        PositionVector.update_lon_scale(self.posvec_home_location.lat)

    # get_home_location - returns home location used by all position vector instances
    @classmethod
    def get_home_location(cls):
        return self.posvec_home_location
    ...
    def update_lon_scale(cls, lat):
        if lat != 0:
            self.posvec_lon_scale = math.cos(math.radians(lat))
